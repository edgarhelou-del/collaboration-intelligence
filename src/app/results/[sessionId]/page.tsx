...
export default function ResultsPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      <p className="text-sm font-medium uppercase tracking-widest text-gold">Resultados</p>
      <h1 className="mt-2 font-serif text-3xl text-paper">Una fotografía de la colaboración en tu organización</h1>
      ...
      <div className="card mt-10 flex flex-col items-center gap-6 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-3 text-sm text-paper/50">Collaboration Score</p>
        </div>
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-2">
        <div className="card">
          <h2 className="font-serif text-xl text-paper">Fortalezas</h2>
          <ul className="mt-4 space-y-3">
            {data.strengths.map((s) => (
              <li key={s.key} className="flex items-center justify-between text-sm">
                <span className="text-paper/80">{s.name}</span>
                <span className="font-medium text-gold">{Math.round(s.value)}</span>
              </li>
            ))}
          </ul>
        </div>
        <div className="card">
          <h2 className="font-serif text-xl text-paper">Áreas de oportunidad</h2>
          <ul className="mt-4 space-y-4">
            {data.opportunities.map((o) => (
              <li key={o.key} className="text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-paper/80">{o.name}</span>
                  <span className="font-medium text-rose">{Math.round(o.value)}</span>
                </div>
                <p className="mt-1 text-paper/50">{o.recommendation}</p>
              </li>
            ))}
          </ul>
        </div>
      ...
      {data.frictions.length > 0 && (
        <div className="card mt-8">
          <h2 className="font-serif text-xl text-paper">Fricciones posibles</h2>
          <ul className="mt-4 space-y-2">
            {data.frictions.map((f, i) => (
              <li key={i} className="rounded-xl bg-rose-light px-4 py-3 text-sm text-paper/80">
                {f}
              </li>
            ))}
          </ul>
